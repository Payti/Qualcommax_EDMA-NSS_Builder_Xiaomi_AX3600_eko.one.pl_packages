...
ECM + SQM; its output lands in the system log (`logread -e nss`). **A reboot
always returns to the stock host-only stack** — the universal recovery path.
...
AX3600 (IPQ8071A, 512 MB), `NSS.FW.12.5-210`, kernel 6.12 — details in the
